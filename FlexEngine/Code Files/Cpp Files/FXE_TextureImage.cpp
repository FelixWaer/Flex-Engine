...
void FXETextureImage::create_TextureImage(const std::string& texturePath)
{
	...
	FXE::create_Image(Device, PhysicalDevice, texWidth, texHeight, MipLevels, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT |
...
{
	TextureImageView = FXE::create_ImageView(Device, TextureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, MipLevels);
}
...
void FXETextureImage::transition_ImageLayout(VkImage image, VkFormat format, VkImageLayout oldImageLayout, VkImageLayout newImageLayout, uint32_t mipLevels)
{
...
void FXETextureImage::copy_BufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
	VkCommandBuffer tempCommandBuffer = FXE::begin_SingleTimeCommands(Device, TheFrameCreationPtr->CommandPool);
	...
	FXE::end_SingleTimeCommands(Device, TheFrameCreationPtr->CommandPool, tempCommandBuffer, GraphicsQueue);
}

void FXETextureImage::generate_Mipmaps(VkImage texImage, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
{
